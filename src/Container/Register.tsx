                ...
                <div>
                    <div className="relative">
                        <Field
                            ...
                            placeholder={('Confirm Password') + ' *'} id="input_password" className='px-2 h-12 mt-4'
                        />
                        {<span className='absolute top-4 right-4 cursor-pointer text-gray-400' onClick={() => { setShowConfirmPassword(!showConfirmPassword) }}>{!showConfirmPassword ? <Icon icon="EYE_SLASH" /> : <Icon icon="EYE" />}</span>}</div>
                </div>
                <div className='justify-center items-center flex flex-col'>
                    <Button className=" py-3 text-white mt-4" color="primary" submit >{`Register`}</Button>
                </div>
                <></>
                <div className='justify-center items-center flex flex-col py-5'>
                    <span className='text-md text-gray-600 cursor-pointer' onClick={() => { navigate('/signIn') }}>
                        Have an account...!
                    </span>
                </div>
            </form>
